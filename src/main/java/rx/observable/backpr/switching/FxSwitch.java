...
public final class FxSwitch extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		Observable<Boolean> selectedStates = JavaFxObservable.valuesOf(startStopButton.selectedProperty()).publish()
				.autoConnect(2);
		
		// Using switchMap() with ToggleButton's selected state will drive whether
		// to kick off an Observable.interval(), or dispose() it by switching to empty Observable
		selectedStates.switchMap(selected -> {
			if (selected)
				...
				.map(Object::toString).subscribe(counterLabel::setText);
